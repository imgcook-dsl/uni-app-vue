  ...
  const generateScss = (schema) => {
    ...
    function walk(json) {
      if (json.props.className) {
        ...
        scss += `${parseStyle(json.props.style)};`
      }
  ...
  const transform = (schema) => {
    ...
    if (Array.isArray(schema)) {
      ...
      if (['page', 'block', 'component'].indexOf(type) !== -1) {
        ...
        if (schema.state) {
          _.forEach(schema.state, (v, k) => {
            datas.push(`${k}=${JSON.stringify(v)}`);
          })
        }
    ...
    panelDisplay: [
      {
        ...
        panelValue: prettier.format(`
          ...
          <script>
              ...
              @Component
  export default class ${schema.componentName} extends Vue {
                ${datas.join('\n')}
                ${lifeCycles.join('\n')}
                ${methods.join('\n')}
  }
